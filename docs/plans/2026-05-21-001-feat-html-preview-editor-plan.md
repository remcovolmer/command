...
- **Sandbox attribute: `allow-scripts allow-forms allow-modals allow-popups`.** Rationale: `allow-scripts` is the minimum for R4; `allow-forms/modals/popups` cover realistic preview content (questionnaires, `alert()`, `window.open()`) without granting access to the host renderer. **`allow-same-origin` is intentionally omitted** — combined with `allow-scripts` on a `srcdoc` iframe it would let previewed HTML reach `window.parent.electronAPI` and call any IPC the preload bridge exposes (fs.writeFile, terminal.create, etc.). Without it the iframe runs in a null origin: asset tags (`<img>`, `<link>`, `<script src>`) still resolve via the injected `<base href>`, but `localStorage`/`sessionStorage`/`document.cookie`/credentialed `fetch` throw `SecurityError`. **`allow-top-navigation` omitted** — prevents the preview from yanking the user away from the editor. **`allow-popups-to-escape-sandbox` omitted** — popups opened by previewed HTML inherit the sandbox, blocking network-exfil paths through `window.open(...)`. The active sandbox string is locked by tests against `IFRAME_SANDBOX` in `src/components/Editor/HtmlPreview.tsx`.
